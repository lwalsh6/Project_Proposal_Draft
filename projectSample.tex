...
For this project, we plan to make a video player program that offers some features that are unavailable in Windows Media Player. 
...
This isn't designed to be some kind of world changing product, rather just one that's convenient to use and may be preferable to Windows Media Player.
...
I believe the two hardest things to make would likely be the
video timeline and adding a fullscreen function, but we will likely be able to add these in with some work. 
...
We'll probably need to break it up into a few different goal we'll have to reach, once all of them are met and 

These will be the basic goals for getting the program started:
1. Design the interface
2. make the program play and video files
3. add skip functions and timeline
4. add a fullscreen function
We will add other features once the program adequately functions as a video player, 
...
We think that this is a worthwhile project because it's not too ambitious and could likely be done in time. On the other hand, making this program will provide some good insight into making a program that's actually practical to use.
...
\end{itemize}
\item The program needs a pause, skip and fullscreen function 
\item a timeline for the video
\item the ability to run multiple formats like MP4s, MKVs and AVIs.

Optional:
\item We could research some more uncommon formats like BIKs to see if we can find a way to get them to run.
\item We could also make it play audio files
